...
/// GPU-accelerated commit: row encoding is dispatched to the GPU.
///
/// Merkle tree construction stays on CPU (sequential hash chain).
pub fn commit_gpu<T: Transcript>(
    evals:      &[GF2_128],
    params:     &PcsParams,
    transcript: &mut T,
    ctx:        &gpu::GpuContext,
    ...
    // GPU-encode all rows
    let code = code::LinearCode::new(k);
    let encoded_rows = gpu_ops::matrix_encode_gpu(ctx, cache, &raw_rows, &code.generated);

    // Hash each column to form Merkle leaves (CPU)
    let leaf_hashes: Vec<Hash> = (0..n_enc)
        .map(|j| {
            let col: Vec<GF2_128> = encoded_rows.iter().map(|row| row[j]).collect();
            hash_column(&col)
...
/// GPU-accelerated open: mixed-row computation dispatched to GPU.
///
/// Column queries and Merkle proofs stay on CPU.
pub fn open_gpu<T: Transcript>(
    state:      &PcsState,
    r:          &[GF2_128],
    transcript: &mut T,
    ctx:        &gpu::GpuContext,
    ...
    // GPU: t_raw[j] = Σ_i eq_hi[i] · M[i][j]
    let t_raw = gpu_ops::batch_row_rlc_gpu(ctx, cache, &state.raw_rows, &eq_hi);
    ...
    // Squeeze query column indices
    let queries: Vec<usize> = (0..params.n_queries)
        .map(|_| (transcript.squeeze_challenge().lo.0 as usize) % n_enc)
        .collect();

    // Build column openings (CPU)
    let column_openings = queries
        .into_iter()
        .map(|j| {
            let col_elems: Vec<GF2_128> = state.encoded_rows.iter().map(|row| row[j]).collect();
...
impl BatchTensorPCS {
    ...
    pub fn commit<T: Transcript>(
        ...
        let raw_rows: Vec<Vec<Vec<GF2_128>>> = self.entries.iter()
            .map(|evals| {
                (0..n_rows)
                    .map(|i| evals[i * k..(i + 1) * k].to_vec())
                    .collect()
...
pub fn batch_open<T: Transcript>(
    ...
    let column_openings = col_queries
        .into_iter()
        .map(|j| {
            let col_elems: Vec<GF2_128> =
                state.encoded_rows.iter().map(|row| row[j]).collect();
            let proof = state.tree.prove(j);
            ColumnOpening { col_idx: j, col_elems, proof }
        })
        .collect();

    (claims, BatchOpenProof { query_opens, column_openings })
}
...
pub fn batch_verify<T: Transcript>(
    ...
    if queries.len() != claims.len() { return false; }
    if queries.len() != proof.query_opens.len() { return false; }
    ...
    for (i, q) in queries.iter().enumerate() {
        if q.point.len() != params.n_vars as usize { return false; }
        if q.entry >= commitment.n_entries { return false; }

        let eq_lo = eq::eq_evals(&q.point[m_rows..]);
        let eq_hi = eq::eq_evals(&q.point[..m_rows]);

        let t_raw = &proof.query_opens[i].t_raw;
        if t_raw.len() != k { return false; }
    ...
    let code  = code::LinearCode::new(k);
    let t_encs: Vec<Vec<GF2_128>> = proof.query_opens.iter()
    ...
    for (opening, &expected_j) in proof.column_openings.iter().zip(col_queries.iter()) {
        if opening.col_idx != expected_j { return false; }
        if opening.col_elems.len() != total_rows { return false; }
...
mod tests {
    ...
    fn batch_single_entry_roundtrip() {
        ...
        let queries = vec![BatchOpenQuery { entry: 0, point: r.clone() }];
        ...
        assert!(batch_verify(&bc, &queries, &claims, &proof, &params, &mut tv));
    }
    ...
    fn batch_multi_entry_roundtrip() {
        ...
        let queries = vec![
            BatchOpenQuery { entry: 0, point: r_a.clone() },
            BatchOpenQuery { entry: 1, point: r_b.clone() },
            BatchOpenQuery { entry: 2, point: r_c.clone() },
        ];
        let (claims, proof) = batch_open(&state, &queries, &mut tp);
    ...
    fn batch_same_entry_two_points() {
        ...
        let queries = vec![
            BatchOpenQuery { entry: 0, point: r1.clone() },
            BatchOpenQuery { entry: 0, point: r2.clone() },
        ];
        let (claims, proof) = batch_open(&state, &queries, &mut tp);
    ...
    fn batch_rejects_wrong_claim() {
        ...
        let queries = vec![
            BatchOpenQuery { entry: 0, point: fixed_point(n_vars, 1) },
            BatchOpenQuery { entry: 1, point: fixed_point(n_vars, 2) },
        ];
    ...
    fn batch_rejects_tampered_column() {
        ...
        let queries = vec![BatchOpenQuery { entry: 0, point: fixed_point(n_vars, 55) }];
    ...
    fn batch_rejects_tampered_t_raw() {
        ...
        let queries = vec![
            BatchOpenQuery { entry: 0, point: fixed_point(n_vars, 3) },
            BatchOpenQuery { entry: 1, point: fixed_point(n_vars, 5) },
        ];
        let (claims, mut proof) = batch_open(&state, &queries, &mut tp);
    ...
    fn batch_n6_five_entries() {
        let n_vars = 6u32;
        let params = PcsParams { n_vars, n_queries: 12 };

        let mut pcs = BatchTensorPCS::new(params.clone());
        let all_evals: Vec<Vec<GF2_128>> = (0..5u64).map(|seed| {
            (0u64..1u64 << n_vars)
                .map(|i| GF2_128::from(i.wrapping_mul(seed.wrapping_add(1).wrapping_mul(6364136223846793005))))
                .collect()
        }).collect();
